...
/******************************************************************************
 template.c: template file for practice
******************************************************************************/
...
#define VMAX 10.0 /* limit of variables in window */
...
void open_window(FILE **gp)
{
  ...
  fprintf(*gp, "set xlabel tc rgb 'white'\n");
  fprintf(*gp, "set ylabel tc rgb 'white'\n");
...
void animated_snapshot(int n, double t, double x[][3], FILE **gp)
{
  ...
  fprintf(*gp, "set object 1 rect behind from screen 0,0 to screen 1,1 fc rgb '#333631' fillstyle solid 1.0\n");
  fprintf(*gp, "plot '-' with points pointtype 0 lc rgb '#ffff00' notitle\n");
  for (i = 0; i <= n; i++)
  {
    fprintf(*gp, "%f\t%f\n", x[i][0], x[i][1]);
  }
...
int main(void)
{
  ...
  */
  ...
  /* change the name of read file if you want */
  FILE *fp;
  fp = fopen("disk1k90_3.ascii", "r");
  ...
  for (int i = 0; i < n; i++){
    fscanf(fp, "%lf%lf%lf", &v[i][0], &v[i][1], &v[i][2]);
  }
  fclose(fp);
